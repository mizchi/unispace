...
const Grid = React__default.memo(function Grid(props) {
    return (React__default.createElement("div", { onDragOver: props.onDragOver, style: Object.assign(Object.assign({}, props.style), { display: "grid", width: props.width || "100%", height: props.height || "100%", overflow: "hidden", gridTemplateColumns: props.columns.join(" "), gridTemplateRows: props.rows.join(" "), gridTemplateAreas: props.areas
                .map(row => "'" + row.join(" ") + "'")
                .join(" ") }) }, props.children));
});
const GridArea = React__default.memo(function GridArea(props) {
...
function HitArea(props) {
    ...
    const [uid] = React.useState(Math.random().toString());
    const [dragging, setDragging] = React.useState(false);
    const onDragStart = React.useCallback(ev => {
        setDragging(true);
        props.onDragStart(ev);
    }, [props.onDragStart]);
    const onDragEnd = React.useCallback(ev => {
        setDragging(false);
        props.onDragEnd(ev);
    }, [props.onDragEnd]);
    return (React__default.createElement("div", { style: {
        ...
        React__default.createElement("div", { className: uid, onDragStart: onDragStart, onDragEnd: onDragEnd, onDrag: props.onDrag, draggable: !props.disabled, style: {
...
function EditableGrid({ width, height, spacerSize, rows, columns, fixedRows, fixedColumns, areas, showCrossPoint = true, showHorizontal = true, showVertical = true, hideOnResize = false, onChangeGridData, onDragStart, onDragEnd, children }) {
    ...
    return (React__default.createElement(EditableGridInner, { original: original, showVertical: showVertical, showHorizontal: showHorizontal, showCrossPoint: showCrossPoint, hideOnResize: hideOnResize, spacerSize: spacerSize, onChangeGridData: onChangeGridData, onDragStart: onDragStart, onDragEnd: onDragEnd }, children));
}
var HoldingType;
...
function EditableGridInner({ original, children, spacerSize, showCrossPoint, showHorizontal, showVertical, hideOnResize, onDragStart: parentOnDragStart, onDragEnd: parentOnDragEnd, onChangeGridData }) {
    ...
    // state
    const [semanticRows, setSemanticRows] = React.useState(original.rows);
    const [semanticColumns, setSemanticColumns] = React.useState(original.columns);
    ...
    return (React__default.createElement(Grid, Object.assign({}, editable, { style: {
        ...
        showVertical &&
            controllers.verticals.map(([row, column]) => {
                ...
                return (React__default.createElement(HitArea, { disabled: disabled, key: name, name: name, onDragStart: onDragStartFactory(HoldingType.Vertical, row, column), onDragEnd: onDragEnd, onDrag: onDrag }));
            }),
        showHorizontal &&
            controllers.horizontals.map(([row, column]) => {
                const name = `h-${row}-${column}`;
                const disabled = !!(original.fixedRows && original.fixedRows[row]);
                return (React__default.createElement(HitArea, { key: name, name: name, disabled: disabled, onDragStart: onDragStartFactory(HoldingType.Horizontal, row, column), onDragEnd: onDragEnd, onDrag: onDrag }));
            }),
        showCrossPoint &&
            controllers.crosses.map(([row, column]) => {
                ...
                return (React__default.createElement(HitArea, { key: name, name: name, disabled: hDisabled || vDisabled, onDragStart: onDragStartFactory(HoldingType.Center, row, column), onDragEnd: onDragEnd, onDrag: onDrag }));
            })));
}
...
function View(props) {
    const { children, style } = props, other = __rest(props, ["children", "style"]);
    return (React__default.createElement("div", Object.assign({ style: Object.assign({ alignItems: "stretch", borderWidth: 0, borderStyle: "solid", boxSizing: "border-box", display: "flex", flexDirection: "column", margin: 0, minHeight: 0, minWidth: 0, padding: 0, position: "relative", zIndex: 0 }, style) }, other), children));
}

const DragContext = React__default.createContext(null);
...
function TabSelector(props) {
    ...
    return (React__default.createElement(View, { onDragEnter: () => {
...
function TabButton(props) {
    ...
    const drag = React.useContext(DragContext);
    const theme = useTheme();
    return (React__default.createElement(Pane, { draggable: true, onClick: props.onClick, onDragEnter: () => {
...
function WindowListContainer({ containerId, windows, selectedId, renderWindow, onSelectTab, onDragStartWindow, onDropWindow, renderTab, showTab }) {
    ...
    return (React__default.createElement(Pane, { style: { flexDirection: "column" } },
        showTab && (React__default.createElement(TabSelector, { windows: windows, renderTab: renderTab, selectedId: selectedId, onDragOver: onDragOverTabbar, onDrop: onDropTabbar, onDropTab: onDropTab, onSelectTab: onSelectTab, onDragStartTab: onDragStartWindow(containerId), onDragOverTab: onDragOverTab })),
        React__default.createElement(Pane, { style: { flex: 1, background: "white", overflowY: "auto" } }, selectedId && renderWindow(selectedId))));
}
...
function EditableLayout(props) {
    ...
    const [dragContextValue, setDragContext] = React.useState(null);
    // Effect
    React.useEffect(() => {
    ...
    const onDropWindow = React.useCallback((dropContainerId, dropWindowId) => (ev) => {
    ...
    const onChangeGridData = React.useCallback((data) => {
    ...
    return (React__default.createElement(DragContext.Provider, { value: dragContextValue },
        React__default.createElement(EditableGrid, Object.assign({ key: `${props.width}-${props.height}`, width: pixelToNumber(props.width), height: pixelToNumber(props.height), spacerSize: (_a = props.spacerSize) !== null && _a !== void 0 ? _a : 8, onChangeGridData: onChangeGridData, onDragStart: props.onDragStart, onDragEnd: props.onDragEnd }, layout.grid), layout.panes.map(pane => {
            var _a;
            const windows = pane.windowIds.map(tid => layout.windows[tid]);
            return (React__default.createElement(GridArea, { name: pane.id, key: pane.id },
                React__default.createElement(WindowListContainer, { showTab: (_a = pane.showTab) !== null && _a !== void 0 ? _a : true, containerId: pane.id, windows: windows, renderTab: props.renderTab, selectedId: pane.selectedId || null, onSelectTab: onSelectTab(pane.id), onDragStartWindow: onDragStartWindow, onDropWindow: onDropWindow, renderWindow: (id) => {
                        if (id) {
...
const DraggingContext = React__default.createContext(false);
function useDragging() {
    return React.useContext(DraggingContext);
}
const WindowManagerContext = React__default.createContext(null);
...
function WindowSelector(props) {
    ...
    if (Current) {
        return (React__default.createElement(React.Suspense, { fallback: "loading..." },
            React__default.createElement(Current, { data: props.data })));
    }
    else {
...
function LayoutSystem(props) {
    ...
    const renderWindow = React.useCallback((win) => {
        return React__default.createElement(WindowSelector, { data: win });
    }, []);
    const renderTab = React.useCallback((data) => {
        return React__default.createElement("span", null, data.displayName);
    }, []);
    return (React__default.createElement(WindowManagerContext.Provider, { value: props.windowManager },
        React__default.createElement(DraggingContext.Provider, { value: dragging },
            React__default.createElement(EditableLayout, { onChangeLayout: props.onChangeLayout, spacerSize: props.spacerSize, width: props.width, height: props.height, layout: props.initialLayout, onDragStart: onDragStart, onDragEnd: onDragEnd, renderTab: renderTab, renderWindow: renderWindow }))));
}
class WindowManager {
    constructor() {
...
exports.Container = WindowListContainer;
exports.EditableGrid = EditableGrid;
exports.EditableLayout = EditableLayout;
exports.Grid = Grid;
exports.GridArea = GridArea;
exports.LayoutSystem = LayoutSystem;
exports.WindowManager = WindowManager;
exports.debounce = debounce;
exports.exprsToPixels = exprsToPixels;
exports.fractionToNumber = fractionToNumber;
exports.interpose = interpose;
exports.numberToFraction = numberToFraction;
exports.numberToPixel = numberToPixel;
exports.pixelToNumber = pixelToNumber;
exports.pixelsToFractions = pixelsToFractions;
